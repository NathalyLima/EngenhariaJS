document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('telefoneFixo').addEventListener('input', aplicarMascaraTelefoneFixo);
    document.getElementById('telefoneCelular').addEventListener('input', aplicarMascaraTelefoneCelular);
    document.getElementById('matriculaProfessor').addEventListener('input', aplicarMascaraMatriculaProfessor);
    document.getElementById('matriculaAluno').addEventListener('input', aplicarMascaraMatriculaAluno);
    
    exibirCamposPerfil('professor');
});

function aplicarMascaraTelefoneFixo() {
    ...
    telefoneFixo.value = telefoneFixo.value
        ...
        .replace(/^(\d{2})(\d)/, "($1)$2") // Coloca parênteses ao redor do código de área
        .replace(/(\d{4})(\d)/, "$1-$2") // Adiciona o hífen depois dos 4 primeiros dígitos
        .slice(0, 13); // Limita a 13 caracteres
}

function aplicarMascaraTelefoneCelular() {
    const telefoneCelular = document.getElementById('telefoneCelular');
    telefoneCelular.value = telefoneCelular.value
        .replace(/\D/g, '') // Remove caracteres que não são dígitos
        .replace(/^(\d{2})(\d)/, "($1)$2") // Coloca parênteses ao redor do código de área
        .replace(/(\d{5})(\d)/, "$1-$2") // Adiciona o hífen depois dos 5 primeiros dígitos
        .slice(0, 14); // Limita a 14 caracteres
}

function aplicarMascaraMatriculaProfessor() {
    const matriculaProfessor = document.getElementById('matriculaProfessor');
    matriculaProfessor.value = matriculaProfessor.value
        .replace(/\D/g, '') // Remove caracteres que não são dígitos
        .slice(0, 5); // Limita a 5 dígitos
}

function aplicarMascaraMatriculaAluno() {
    const matriculaAluno = document.getElementById('matriculaAluno');
    matriculaAluno.value = matriculaAluno.value
        .replace(/\D/g, '') // Remove caracteres que não são dígitos
